Ext.define('Rally.technicalservices.HierarchyLoader', {
    ...
    fetchPortfolioItems: function(type, parentRecords) {
        console.log('fetchPortfolioItems', type);
    ...
    fetchWsapiRecords: function(config) {
        ...
        console.log(config);
        if (config.model == 'portfolioitem/feature'){
            config.types = 'portfolioitem/feature'
        }
        else if (config.model == 'portfolioitem/epic'){
            config.types = 'portfolioitem/epic'
        }

        var store = Ext.create('Rally.data.wsapi.Store', config);
        store.projectRef = this.projectRef;
        store.on('beforeload', this.fnBeforeLoadStore);
        store.load({
